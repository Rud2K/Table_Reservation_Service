...
public class UserController {
  ...
  /**
   * 회원가입 요청을 처리하는 메소드 POST /auth/signup
   * 
   * @param request 회원가입 요청 정보 (User.SignUp)
   * @return 생성된 UserEntity 객체를 포함한 ResponseEntity
   * 
   * 클라이언트로부터 회원가입 정보를 받아 UserService를 통해 처리한 후, 생성된 사용자 정보를 반환합니다.
   */
  @PostMapping("/signup")
  ...
  /**
   * 로그인 요청을 처리하는 메소드 POST /auth/signin
   * 
   * @param request 로그인 요청 정보 (User.SignIn)
   * @return 로그인 성공 시 JWT 정보를 포함한 ResponseEntity
   * 
   * 클라이언트로부터 로그인 정보를 받아 UserService를 통해 처리한 후, 생성된 JWT 정보를 반환합니다.
   */
  @PostMapping("/signin")
